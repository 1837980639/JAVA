...
function config_cmdb($stateProvider, $ocLazyLoadProvider) {
    ...
    }).state('infrastructure.nodegroup', {
        url: "/infrastructure_nodegroup",
        data: {pageTitle: '设备组'},
        templateUrl: "views/cmdb/infrastructure/nodegroup.html?v=" + version,
        resolve: {
            loadPlugin: function ($ocLazyLoad) {
                return $ocLazyLoad.load([{
                    serie: true,
                    files: ['views/cmdb/infrastructure/nodegroup.js?v=' + version]
                }]);
            }
        }
    }).state('infrastructure.nodemap', {
        url: "/infrastructure_nodemap",
        data: {pageTitle: '设备组关联'},
        templateUrl: "views/cmdb/infrastructure/nodemap.html?v=" + version,
        resolve: {
            loadPlugin: function ($ocLazyLoad) {
                return $ocLazyLoad.load([{
                    serie: true,
                    files: ['views/cmdb/infrastructure/nodemap.js?v=' + version]
                }]);
            }
        }
    }).state('infrastructure.sbpz', {
        url: "/infrastructure_sbpz",
        data: {pageTitle: '设备凭证'},
        templateUrl: "views/cmdb/infrastructure/sbpz.html?v=" + version,
        resolve: {
            loadPlugin: function ($ocLazyLoad) {
                return $ocLazyLoad.load([{
                    serie: true,
                    files: ['views/cmdb/infrastructure/sbpz.js?v=' + version]
                }]);
            }
        }
    }).state('infrastructure.pzdj', {
        url: "/infrastructure_pzdj",
        data: {pageTitle: '凭证登记'},
        templateUrl: "views/cmdb/infrastructure/pzdj.html?v=" + version,
        resolve: {
            loadPlugin: function ($ocLazyLoad) {
                return $ocLazyLoad.load([{
                    serie: true,
                    files: ['views/cmdb/infrastructure/pzdj.js?v=' + version]
                }]);
            }
        }
    });
...
function modal_common_ZcListCtl($timeout, $localStorage, notify, $log, $uibModal,
    ...
    if(angular.isDefined(data.datarange) && data.datarange=="SBPZ"){
        var col={};
        col.ip="Y";
        $scope.dtColumns = assetsBaseColsCreate(DTColumnBuilder, 'withselect',col);
    }else{
