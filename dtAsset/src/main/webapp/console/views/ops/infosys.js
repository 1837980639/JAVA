...
//				}
    ...
    $scope.systp = function () {
        ...
        if (angular.isDefined(selrow)) {
            $window.open("views/ops/tp/sysinfotp.html?id="+selrow.id);
        }
    }
    $scope.devtp = function () {
        var selrow = getSelectRow();
        if (angular.isDefined(selrow)) {
            $http.post($rootScope.project + "/api/ops/opsNodegroup/ext/queryGroupIdsBySystemId.do", {
                id:selrow.id
            }).success(function (res) {
                if (res.success) {
                    $window.open($rootScope.project+"/console/views/cmdb/infrastructure/nodegroupshow.html?id="+res.data);
                } else {
                    notify({
                        message: res.message
                    });
