    ...
    if (angular.isDefined(gdicts.sysloc) && gdicts.sysloc.length > 0) {
        ...
        if (angular.isDefined(item) && angular.isDefined(item.loc)) {
            for (var i = 0; i < modal_meta.meta.locOpt.length; i++) {
                if (modal_meta.meta.locOpt[i].id == item.loc) {
...
function syshostmgrCtl(DTOptionsBuilder, DTColumnBuilder, $compile, $confirm,
    ...
    var dicts = "nodebak,sysstatus,sysdb,sysdbdtl,sysenv,sysexecenv,syslevel,sysmid,sysmonitor,sysos,sysosdtl,syspwdstrategy,systype";
    ...
    $scope.dtColumns.push(DTColumnBuilder.newColumn('ip').withTitle('IP')
        .withOption('sDefaultContent', '').withOption("width", '30'));
    $scope.dtColumns.push(DTColumnBuilder.newColumn('oip').withTitle('虚IP')
        .withOption('sDefaultContent', '').withOption("width", '30'));
    ...
    $scope.dtColumns.push(DTColumnBuilder.newColumn('confdesc').withTitle('配置信息')
        .withOption('sDefaultContent', ''));
    ...
    var meta = {
        ...
        toolsbtn: [
            ...
            },
            {
                ...
                show: true,
                priv: "remove",
                template: ' <button ng-click="systp()" class="btn btn-sm btn-primary" type="submit">系统拓扑</button>'
            },
            {
                id: "btn22",
                label: "",
                ...
                show: true,
                priv: "remove",
                template: ' <button ng-click="devtp()" class="btn btn-sm btn-primary" type="submit">设备拓扑</button>'
            },
            {
                id: "btn3",
                label: "",
    ...
    $scope.systp = function () {
        var selrow = getSelectRow();
        if (angular.isDefined(selrow)) {
            $http.post($rootScope.project + "/api/ops/opsNodeSyshostmap/ext/querySysIdByHostId.do", {
                id:selrow.id
            }).success(function (res) {
                if (res.success) {
                   $window.open("views/ops/tp/sysinfotp.html?id="+res.data.sysid);
                } else {
                    notify({
                        message: res.message
                    });
    ...
    $scope.devtp = function () {
        var selrow = getSelectRow();
        if (angular.isDefined(selrow)) {
            $http.post($rootScope.project + "/api/ops/opsNodegroup/ext/queryGroupIdByHostId.do", {
               id:selrow.id
            }).success(function (res) {
                if (res.success) {
                    $window.open($rootScope.project+"/console/views/cmdb/infrastructure/nodegroupview.html?id="+res.data);
                } else {
                    notify({
                        message: res.message
                    });
    ...
    $scope.save = function (type) {
        ...
        meta = {
            ...
            sure: function (modalInstance, modal_meta) {
                ...
                if (angular.isDefined(modal_meta.meta.locSel) && angular.isDefined(modal_meta.meta.locSel.id)) {
                    modal_meta.meta.item.loc = modal_meta.meta.locSel.id
                }
