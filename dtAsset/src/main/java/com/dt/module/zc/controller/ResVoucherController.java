 ...
 */
@Controller
@RequestMapping("/api/zc/resVoucher")
public class ResVoucherController extends BaseController {
	...
	public R insert(ResVoucher entity) {
		return R.SUCCESS_OPER(ResVoucherServiceImpl.save(entity));
	}

	@ResponseBody
	@Acl(info = "根据Id更新", value = Acl.ACL_USER)
	@RequestMapping(value = "/updateById.do")
	public R updateById(ResVoucher entity) {
		return R.SUCCESS_OPER(ResVoucherServiceImpl.updateById(entity));
	}
	...
	public R selectList() {
		return R.SUCCESS_OPER(ResVoucherServiceImpl.list(null));
	}
	...
	public R selectPage(String start, String length, @RequestParam(value = "pageSize", required = true, defaultValue = "10")  String pageSize,@RequestParam(value = "pageIndex", required = true, defaultValue = "1")  String pageIndex) {
		...
		QueryWrapper<ResVoucher> ew = new QueryWrapper<ResVoucher>();
		//ew.and(i -> i.eq("user_id", getUserId()).apply(pagesize>10, "rtime>sysdate-1","23"));
		IPage<ResVoucher> pdata = ResVoucherServiceImpl.page(new Page<ResVoucher>(pageindex, pagesize), ew);
