...
public class ApplicationContextListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        if (null == event.getApplicationContext().getParent()) {
            ...
            //处理ctl
            Rcd vrs = DB.instance().uniqueRecord("select * from sys_params where id=?", "ctl");
            if(vrs==null){
                JSONObject obj=new JSONObject();
                Insert ins=new Insert("sys_params");
                ins.set("id","ctl");
                ins.set("type","system");
                Date dNow = new Date( );
                SimpleDateFormat ft = new SimpleDateFormat ("yyyy-MM-dd");
                String nowStr=ft.format(dNow);
                obj.put("record",nowStr);
                obj.put("message","当前为开源版本(https://gitee.com/lank/zcdevmgr),欢迎使用最新EAM社区版！");
                System.out.println(obj.toJSONString());
                String md5=MD5Util.encrypt(obj.toJSONString()+"kl");
                ins.set("value",obj.toJSONString());
                ins.set("mark",md5);
                DB.instance().execute(ins);
            }else{
