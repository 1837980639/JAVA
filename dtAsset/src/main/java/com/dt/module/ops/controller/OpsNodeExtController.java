...
public class OpsNodeExtController extends BaseController {
    ...
    @ResponseBody
    @Acl(info = " ", value = Acl.ACL_USER)
    @RequestMapping(value = "/selectArchList.do")
    public R selectArchList(String search) {
        return opsNodeService.selecArchList(search);
    }
    ...
    public R selectDBListSimple() {
        String sql = "select concat(dbtype,\"_\",name) dbname,id from (select name,id,"
                ...
                + "  from ops_node t where arch='0' and dr='0' and db is not null and length(db)>2) end order by 1";
